...
const editar = async (req, res) => {
    ...
    if(error) {
        ...
        console.log("Institucion editar");
        ...
        let institucionData;
        try {
            institucionData = await Institucion.findOne({
                attributes: ['codigo_snies'],
                where: [{
                    id: req.params.id
                }]
            });
        ...
        if(institucionData) {
            let lastCodigoSnies = institucionData['codigo_snies'];

            // Editar la institución con id recibido
            let institucion;
            try {
              	institucion = await Institucion.update(
                        ...
                        //{ where: { codigo_snies: req.body.codigoSnies } }
                );
            }
            catch (error) {
                console.log(" Inst");
            ...
            let institucionOtras;
            try {
                  institucionOtras = await Institucion.update(
                        { 
                            ...
                            sector: sector, 
                            acreditacion: acreditacion, 
                            //es_principal: req.body.esPrincipal, 
                            fecha_registro: fechaRegistro, 
                            telefono_contacto: telefonoContacto, 
                            direccion_domicilio: direccionDomicilio, 
                            programas_vigentes: programasVigentes,
                        ...
                        { where: { codigo_snies: lastCodigoSnies } }
                );
            }
            catch (error) {
                console.log(" Inst");
